    ...
    <div class="col-md-9">
        ...
        <div class="card p-4 shadow-sm border-0" id="recommended-jobs">
            ...
            <?php if (empty($recommendedJobs)): ?>
                <div class="alert alert-info small border-0 shadow-sm">Aktuálně nemáme žádné doporučené pozice pro tuto akci.</div>
            <?php else: ?>
                <div class="row g-3">
                        ...
                        <div class="col-12">
                            <div class="card border p-3">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        <h5 class="mb-1 text-primary fw-bold small"><?= e($job['title']) ?></h5>
                                        <p class="mb-0 small text-muted"><strong><?= e($job['company_name']) ?></strong> | <?= e($job['location']) ?></p>
                                    </div>
                                    <span class="badge badge-<?= $job['match_color'] ?> rounded-pill px-3 py-2 small">Shoda <?= $job['match_score'] ?>%</span>
                                </div>
                                <div class="mt-3 text-end">
                                    <a href="/company_detail.php?id=<?= $job['company_id'] ?>" class="btn btn-sm btn-outline-primary rounded-pill">Detail firmy</a>
                                    <a href="/meeting_request.php?job_id=<?= $job['id'] ?>" class="btn btn-sm btn-success rounded-pill ms-2 text-white">Schůzka</a>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
